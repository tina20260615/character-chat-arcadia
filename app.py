...
- 지문(나레이션)은 항상 3인칭으로 써라 - 절대 규칙. "아젤은 그녀를 바라보았다"처럼
  인물의 이름이나 "그/그녀"로 지칭하고, "나는 그녀를 바라보았다", "내 머릿속은 온통
  그녀 생각뿐이었다"처럼 특정 인물의 1인칭 시점으로 지문을 쓰지 마라. 1인칭 "나는/내가"는
  오직 큰따옴표 대사 안에서만, 또는 편지·일기처럼 그 인물이 직접 쓴 글이라는 게 형식상
  명백할 때만 예외로 허용된다.
